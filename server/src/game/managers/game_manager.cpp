...
void GameManager::PlacePlayers(const RoomManager::RoomSnapshot& snapshot,
  ...
  const float center_x = static_cast<float>(scene->config.width) * 0.5f; // 计算中心x
  const float center_y = static_cast<float>(scene->config.height) * 0.5f; // 计算中心y
...
bool GameManager::HandlePlayerInput(uint32_t player_id,
  ...
  const float len = std::sqrt(dx_raw * dx_raw + dy_raw * dy_raw); // 计算位移长度
