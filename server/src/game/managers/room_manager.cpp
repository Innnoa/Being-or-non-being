...
lawnmower::S2C_CreateRoomResult RoomManager::CreateRoom( // 创建房间
...

  {
    ...
    update = BuildRoomUpdateLocked(iter->second); // 房间发生变化,更新房间基本信息
...
lawnmower::S2C_LeaveRoomResult RoomManager::LeaveRoom(uint32_t player_id) { // 离开房间
...
bool RoomManager::DetachPlayerLocked(uint32_t player_id, RoomUpdate* update) {
  auto mapping = player_room_.find(player_id); // 通过player_id 查找 room_id 是否存在
  if (mapping == player_room_.end()) {
